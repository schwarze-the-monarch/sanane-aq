...
exports.run = async(client, message, args, ayar, emoji) => {
 if (!message.member.roles.cache.has("764483384608817172") && !message.member.hasPermission('BAN_MEMBERS')) return message.channel.send(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor(0x00bfff).addField("Yetersiz Yetki",`Bu Komutu Kullanmak içi Yeterli Yetkiniz Yok`)).then(m => m.delete({timeout: 7000}));
  ...
  client.channels.cache.get("779759458674540585").send(new Discord.MessageEmbed().setColor(0x00bfff).setDescription(`<@!${victim.user.id}> İsimli Kullanıcı Sunucudan Banlandı\n\n• Yetkili: <@!${message.author.id}> \`${message.author.id}\`\n• Banlanan Üye: <@!${victim.user.id}> \`${victim.user.id}\`\n\n• Sebep: **${reason}**`));
};
exports.conf = {
    enabled: true,
    guildOnly: false,
